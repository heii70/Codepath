<?php
  ...
  function insert_failed_login($user) {
    global $db;
    $sql = "INSERT INTO failed_logins ";
    $sql .= "(username, count, last_attempt) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $user['username']) . "', ";
    $sql .= "'" . db_escape($db, $user['count']) . "', ";
    $sql .= "'" . db_escape($db, $user['last_attempt']) . "'";
    $sql .= ");";
    $users_result = db_query($db, $sql);
    if($users_result) {
      return true;
  ...
  function update_failed_login($user) {
    global $db;
    $sql = "UPDATE failed_logins SET ";
    $sql .= "count='" . db_escape($db, $user['count']) . "', ";
    $sql .= "last_attempt='" . db_escape($db, $user['last_attempt']) . "' ";
  ...
  function throttle_time($username) {
    ...
    $remaining_lockout = ceil($lockout - $since_last_attempt);
