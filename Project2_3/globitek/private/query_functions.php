<?php
  ...
  function insert_country($country) {
    ...
    $sql .= "(name, code) ";
    $sql .= "VALUES (";
    $sql .= "'" . escape($country['name']) . "',";
    $sql .= "'" . escape($country['code']) . "'";
    $sql .= ");";
  ...
  function validate_country($country, $errors=array()) {
    ...
    } elseif (!is_alphabetic($country['code'])) {
      $errors[] = "Code format is invalid.";
    }
  ...
  function update_country($country) {
    ...
    $sql .= "name='" . escape($country['name']) . "', ";
    $sql .= "code='" . escape($country['code']) . "' ";
    $sql .= "WHERE id='" . escape($country['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function find_states_for_country_id($country_id=0) {
    global $db;
    $sql = "SELECT * FROM states ";
    $sql .= "WHERE country_id='" . escape($country_id) . "' ";
    $sql .= "ORDER BY name ASC;";
    $state_result = db_query($db, $sql);
    return $state_result;
  }
  ...
  function validate_state($state, $errors=array()) {  
    ...
    if (is_blank($state['code'])) {
      ...
      $errors[] = "Code must be exact 2 characters.";
    } elseif (!is_alphabetic($state['code'])) {
      $errors[] = "Code format is invalid.";
    }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country ID cannot be blank.";
    } elseif (!is_numeric($state['country_id']) || $state['country_id'] < 0) {
      $errors[] = "Country ID format is invalid.";
    }
  ...
  function insert_state($state) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . escape($state['name']) . "',";
    $sql .= "'" . escape($state['code']) . "',";
    $sql .= "'" . escape($state['country_id']) . "'";
    $sql .= ");";
  ...
  function update_state($state) {
    ...
    $sql .= "name='" . escape($state['name']) . "', ";
    $sql .= "code='" . escape($state['code']) . "', ";
    $sql .= "country_id='" . escape($state['country_id']) . "' ";
    $sql .= "WHERE id='" . escape($state['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function find_territories_for_state_id($state_id=0) {
    global $db;
    $sql = "SELECT * FROM territories ";
    $sql .= "WHERE state_id='" . escape($state_id) . "' ";
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    if (is_blank($territory['position'])) {
      ...
      $errors[] = "Position must have no more than 255 characters.";
  ...
  function insert_territory($territory) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . escape($territory['name']) . "',";
    $sql .= "'" . escape($territory['state_id']) . "',";
    $sql .= "'" . escape($territory['position']) . "'";
    $sql .= ");";
  ...
  function update_territory($territory) {
    ...
    $sql .= "name='" . escape($territory['name']) . "', ";
    $sql .= "state_id='" . escape($territory['state_id']) . "', ";
    $sql .= "position='" . escape($territory['position']) . "' ";
    $sql .= "WHERE id='" . escape($territory['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function find_salespeople_for_territory_id($territory_id=0) {
    ...
    $sql .= "WHERE salespeople_territories.territory_id='" . escape($territory_id) . "' ";
    $sql .= "ORDER BY last_name ASC, first_name ASC;";
    $salespeople_result = db_query($db, $sql);
    return $salespeople_result;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    if (is_blank($salesperson['first_name'])) {
      ...
      $errors[] = "First name format is invalid.";
    }
    ...
    if (is_blank($salesperson['last_name'])) {
      ...
      $errors[] = "Last name format is invalid.";
    }
  ...
  function insert_salesperson($salesperson) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . escape($salesperson['first_name']) . "',";
    $sql .= "'" . escape($salesperson['last_name']) . "',";
    $sql .= "'" . escape($salesperson['email']) . "',";
    $sql .= "'" . escape($salesperson['phone']) . "'";
    $sql .= ");";
  ...
  function find_territories_by_salesperson_id($id=0) {
    ...
    $sql .= "WHERE salespeople_territories.salesperson_id='" . escape($id) . "' ";
  ...
  function find_user_by_id($id=0) {
    global $db;
    $sql = "SELECT * FROM users WHERE id='" . escape($id) . "' LIMIT 1;";
    $users_result = db_query($db, $sql);
    return $users_result;
  }
  ...
  function insert_user($user) {
    ...
    $sql .= "VALUES (";
    $sql .= "'" . escape($user['first_name']) . "',";
    $sql .= "'" . escape($user['last_name']) . "',";
    $sql .= "'" . escape($user['email']) . "',";
  ...
  function update_user($user) {
    ...
    $sql .= "first_name='" . escape($user['first_name']) . "', ";
    $sql .= "last_name='" . escape($user['last_name']) . "', ";
    $sql .= "email='" . escape($user['email']) . "', ";
    $sql .= "username='" . escape($user['username']) . "' ";
    $sql .= "WHERE id='" . escape($user['id']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  function delete_user_by_id($id=0) {
    global $db;

    $sql_1 = "DELETE FROM users WHERE id=" . escape($id) . " LIMIT 1;";
    $sql_2 = "ALTER TABLE users AUTO_INCREMENT=0;";

    $result = db_query($db, $sql_1);
    if($result)
    {
      $result = db_query($db, $sql_2);
      if($result)
        return true;
    }
