...
Bonus Objective 1 Answer:

The login portion of this assignment has a major security weakness due to it not following the principle of Security through obscurity. The page gives too much information back to user when a login fails. A hacker can use this information to narrow down their point of target. In addition, the master password used is unencrypted which also jeopardizes the security of the database. The code can be modified so that it gives less and more general information to the user when login fails. The master password should also be encrypted.

Describe any challenges encountered while building the app.

I had a major issue where the session id would be lost after the redirect. After some time researching, I found out that the session.secure_cookies setting in the php.ini file was set to 1, which caused the server to ignore any cookies not sent using https. I set the setting to 0 and resolved the session id issue. 
